...
function InventoryForm() {
    ...
    useEffect(() => {
        const fetchThreadId = async () => {
            try {
                const response = await axios.get('http://localhost:3002/check-access');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            let exlcthread = await axios.get('http://localhost:3002/check-access');
            setThreadId(exlcthread.data.threadId);
            if (threadId === null || threadId === localStorage.getItem('threadId')) {
                try {
                    const resp = await axios.post('http://localhost:3002/grant-access', { threadId: localStorage.getItem('threadId') });
                    console.log('Response:', resp.data);

                    const response = await axios.post(`http://localhost:3002/api/inventory/${endpoint}`, {
                        protocol: protocol,
                        bloodGroup: itemName,
    ...
    const handleCheckboxChange = async (event) => {
        if (isChecked) {
            ...
            try {
                const response = await axios.post('http://localhost:3002/grant-access', { threadId: null });
        ...
        try {
            const accessCheckResponse = await axios.get('http://localhost:3002/check-access');
            ...
            if (accessCheckResponse.data.threadId === null || accessCheckResponse.data.threadId === localStorage.getItem('threadId')) {
                const grantAccessResponse = await axios.post('http://localhost:3002/grant-access', { threadId: localStorage.getItem('threadId') });
